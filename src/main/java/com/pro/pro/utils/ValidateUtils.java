...
public class ValidateUtils {
    ...
    public static void validateAge(Integer age) {
        if (Objects.isNull(age))
            throw new ApiRequestExcept("Field Age is wrong or null");
        if (age < 0)
            throw new ApiRequestExcept("Age can not be negative");
    }

    public static void validateEducationLevel(ProDTO proDTO) {
        if (Objects.isNull(proDTO.getEducation_level()))
            throw new ApiRequestExcept("Field Education Level is wrong or not exist");
    ...
    public static void validateInternetTest(ProDTO proDTO) {
        if (Objects.isNull(proDTO.getInternet_test()))
            throw new ApiRequestExcept("Internet Test is wrong or not exist");
    }

    public static void validadeWritingScore(ProDTO proDTO) {
        if (Objects.isNull(proDTO.getWriting_score()))
            throw new ApiRequestExcept(("Writing Score is wrong or not exist"));
